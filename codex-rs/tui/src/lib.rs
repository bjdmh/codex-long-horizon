...
pub async fn run_main(mut cli: Cli, arg0_paths: Arg0DispatchPaths) -> std::io::Result<AppExitInfo> {
    ...
    let self_directed_innovation_instructions = "Bounded self-directed innovation is enabled for this Non-stop run. Treat the active goal as an end-to-end delivery contract. If you can identify a concrete next step that fits in this turn, execute it instead of ending with suggestions, remaining-work notes, or a recommendation list. Use <task_complete>...</task_complete> only if you have actively searched for the next step, exhausted the useful actions that fit in this turn, and there is truly nothing meaningful left to do here except deliberate handoff while the overall goal still remains active. You may only use self-directed innovation when it clearly stays within the active goal, fits the remaining time budget, and does not create obvious high-risk side effects. Before executing any self-directed innovation, first record it with <innovation_candidate>{\"title\":\"...\",\"rationale\":\"...\",\"relevance\":\"...\",\"risk\":\"low|medium|high\",\"estimated_duration\":\"30m\"}</innovation_candidate> and then hand off with <task_complete>...</task_complete>.";
