...
mod tests {
    ...
    #[tokio::test]
    async fn exec_non_stop_user_turn_does_not_inject_interactive_override() {
        let mut session_configuration = make_session_configuration_for_tests().await;
        session_configuration.session_source = SessionSource::Exec;
        session_configuration.collaboration_mode = CollaborationMode {
            mode: ModeKind::NonStop,
            settings: Settings {
                model: "gpt-5.4".to_string(),
    ...
    #[tokio::test]
    async fn interactive_non_stop_override_is_not_duplicated_across_explicit_turns() {
        let mut session_configuration = make_session_configuration_for_tests().await;
        session_configuration.session_source = SessionSource::Cli;
        session_configuration.collaboration_mode = CollaborationMode {
            mode: ModeKind::NonStop,
        ...
        handlers::user_input_or_turn(&session, "sub-id-1".to_string(), make_turn()).await;
        handlers::user_input_or_turn(&session, "sub-id-2".to_string(), make_turn()).await;
        ...
        assert_eq!(
            instructions
                .matches(INTERACTIVE_NON_STOP_NEW_USER_TURN_INSTRUCTIONS)
                .count(),
            1
        );
    }
