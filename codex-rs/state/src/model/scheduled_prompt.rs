...
pub enum ScheduledPromptKind {
    Loop,
    Task,
}

impl ScheduledPromptKind {
    pub const fn as_str(self) -> &'static str {
        match self {
            ScheduledPromptKind::Loop => "loop",
            ScheduledPromptKind::Task => "task",
        }
    }

    fn parse(value: &str) -> Result<Self> {
        match value {
            "loop" => Ok(ScheduledPromptKind::Loop),
            "task" => Ok(ScheduledPromptKind::Task),
            _ => Err(anyhow::anyhow!("invalid scheduled prompt kind: {value}")),
        }
    }
}
...
impl ScheduledPromptStatus {
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            ScheduledPromptStatus::Paused => "paused",
            ScheduledPromptStatus::Completed => "completed",
...
impl TryFrom<ScheduledPromptRow> for ScheduledPrompt {
    ...
    fn try_from(value: ScheduledPromptRow) -> Result<Self, Self::Error> {
        ...
        } else if value.completed_at.is_some() {
            ScheduledPromptStatus::Completed
        } else if value.paused_until.is_some() {
            ScheduledPromptStatus::Paused
        } else {
            ScheduledPromptStatus::Active
        };
        let kind = ScheduledPromptKind::parse(&value.kind)?;
