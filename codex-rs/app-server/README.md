...
- `schedule/create` — create a recurring scheduled loop for a stored thread.
- `schedule/list` — list scheduled items, optionally filtered by `threadId`. This includes recurring loops and one-shot future tasks created by Codex.
- `schedule/cancel` — cancel a scheduled item by id.
...
### Example: Create a recurring scheduled loop
