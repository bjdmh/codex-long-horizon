...
- `turn/start` — add user input to a thread and begin Codex generation; responds with the initial `turn` object and streams `turn/started`, `item/*`, and `turn/completed` notifications. If `collaborationMode` is omitted, Codex uses the thread's current collaboration mode, which defaults to `execute` for new sessions unless configured otherwise. Built-in modes currently include `default`, `execute`, `non_stop`, and `plan`. For `collaborationMode`, `settings.developer_instructions: null` means "use built-in instructions for the selected mode".
